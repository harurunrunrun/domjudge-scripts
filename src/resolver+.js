...
(() => {
  ...
  function validateFinal(finalState) {
    ...
    const actualIds = new Set(actualMap.keys());
    const ourIds = new Set(ours);
    const missing = [...ourIds].filter((id) => !actualIds.has(id));
    const extra = [...actualIds].filter((id) => !ourIds.has(id));
    const mismatch = [];

    for (const tid of ours) {
      const a = actualMap.get(tid), s = finalState.standings.get(tid);
      if (!a || !s) continue;
      if (Number(a.numSolved) !== Number(s.numSolved)) mismatch.push(`${tid}: solved ${s.numSolved} != ${a.numSolved}`);
      ...
      if (!(at === s.time || at === s.time * 60)) mismatch.push(`${tid}: time ${s.time} != ${at}`);
      // Exact row order inside a tied rank may differ because DOMjudge/MySQL and the browser use
      // different collation implementations. Rank is what matters for the ICPC standing.
      if (String(a.rank) !== String(s.rank)) mismatch.push(`${tid}: rank ${s.rank} != ${a.rank}`);
    }

    if (missing.length || extra.length || mismatch.length) {
      const lines = [
        'DOMjudge の最終順位表とブラウザ側の ICPC モデルが一致しません。誤った resolve を表示しないため停止しました。',
        ...(missing.length ? [`scoreboard にない内部チーム: ${missing.slice(0, 8).join(', ')}`] : []),
        ...(extra.length ? [`内部モデルにない scoreboard チーム: ${extra.slice(0, 8).join(', ')}`] : []),
        ...mismatch.slice(0, 8),
      ];
  ...
  async function loadContest(cid) {
    ...
    const scoreboardTeamIds = new Set(finalRows.map((r) => r.teamId).filter(Boolean));
    if (!scoreboardTeamIds.size) throw new Error('Final scoreboard is empty.');
    ...
    // Participation filter: a team is considered to have participated if either:
    //   1) the submissions endpoint contains at least one in-contest submission, OR
    //   2) DOMjudge's final scoreboard reports at least one judged/pending attempt.
    // Using both sources is intentional. Some DOMjudge configurations/API versions may omit
    // submissions from one representation (for example non-scoring compiler errors), while the
    // other source still proves that the team submitted. Verdict/solved status is irrelevant:
    // WA/CE-only teams must remain visible. Only teams with truly zero submissions are removed.
    // Contest time is the half-open interval [0, duration).
    const teamsWithContestSubmission = new Set();
    for (const sub of model.submissions) {
      if (sub.contestTime >= 0 && sub.contestTime < model.durationMs) {
        teamsWithContestSubmission.add(sub.teamId);
      }
    }
    for (const row of finalRows) {
      if (row.problems.some((p) => Number(p.numJudged || 0) > 0 || Number(p.numPending || 0) > 0)) {
        teamsWithContestSubmission.add(row.teamId);
      }
    }
    model.finalTeamIds = new Set(
      finalRows.map((r) => r.teamId).filter((teamId) => teamsWithContestSubmission.has(teamId))
    );
    if (!model.finalTeamIds.size) throw new Error('コンテスト時間中に提出した scoreboard 対象チームがありません。');
    ...
    model.awards = arr(awardsRaw, 'awards').map((a) => ({
      ...
      teamIds: (pick(a, 'team_ids', 'teamIds') || []).map(String).filter((teamId) => model.finalTeamIds.has(teamId)),
